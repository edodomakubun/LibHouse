...
import { materials as materialsTable, categories as categoriesTable } from "@/db/schema";
...
export default async function ExplorePage() {
  ...
  try {
    ...
    dynamicCategories = await db.select().from(categoriesTable).all();
  } catch (e) {
    console.error("Failed to fetch materials:", e);
    // Fallback to mock data if DB is not ready or in dev without local D1
    materials = [
      { id: "1", title: "Kalkulus Dasar Semester 1", description: "", fileKey: "", category: "MIPA", course: "Kalkulus 1", userId: "dosen_killer", upvotesCount: 124, isAnonymous: false, createdAt: new Date() },
    ] as any[];
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="flex gap-4 mb-12 overflow-x-auto pb-4 no-scrollbar">
        <button
          className="px-6 py-2.5 rounded-full font-black text-sm whitespace-nowrap transition-all border-2 bg-black text-white border-black"
        >
          Semua
        </button>
        {dynamicCategories.length > 0 ? (
          dynamicCategories.map((cat) => (
            <button
              key={cat.id}
              className="px-6 py-2.5 rounded-full font-black text-sm whitespace-nowrap transition-all border-2 bg-white border-black/5 hover:border-pastel-purple"
            >
              {cat.name}
            </button>
          ))
        ) : (
          ["Teknik", "Ekonomi", "Hukum", "Kedokteran", "Sospol", "MIPA"].map((cat) => (
            <button
              key={cat}
              className="px-6 py-2.5 rounded-full font-black text-sm whitespace-nowrap transition-all border-2 bg-white border-black/5 hover:border-pastel-purple"
            >
